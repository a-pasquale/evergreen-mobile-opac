...
require [
  ...
  $ ->
    ...
    # If id is set on the index page, try to create a new session.
    if window.active_id?
      require ['eg/eg_api'], (eg) ->
        ...
        , (resp) ->
          ...
          $().publish 'session.login', [window.active_name]
        return
    ...
    $('#manage_account').subscribe 'session.login', ->
    ...
    $('body').on 'click', '.manage_account_link', ->
      console.log("clicked on manage_account")
      $('#manage_account').show()
      $('body').pagecontainer('change', '#manage_account', {transition: 'slide'})
      require ['manage_account'], =>
        $('#card_list').manage_account()
      return false
    ...
    $('body').on 'pageshow', (e) ->
      t = e.target
      switch t.id
        when 'account_login' then $('form input:eq(0)', t).focus()
